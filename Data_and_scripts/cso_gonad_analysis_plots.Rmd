...
cso_dat_lfm <- 
  cso_dat_lfm %>%
  mutate(c_llength = llength - mean(llength))

#convert whole weight estiamtes to grams, because fishbase a, b parameters in cm and grams, so change our units to make comparable
cso_dat_lfm$whole_wt_g<-cso_dat_lfm$whole_wt_kg*1000 ###multiply by 1000
cso_dat_lfm$lweight_g<-log(cso_dat_lfm$whole_wt_g)
...
##add fulton's condition index (K = 100 * W/L^3, where W is wet weight grams, L is length in cm)
cso_dat_lfm <- 
  cso_dat_lfm %>%
  mutate(fulton_k = 100*whole_wt_g/(fork_length_cm^3))
...
gsi_length_modpost <- posterior_samples(gsi_length_mod)
median_hdi(gsi_length_modpost$b_treatment_b, .width = .95) #-0.2523401	-0.4472239	-0.06602361	
median_hdi(gsi_length_modpost$b_Intercept, .width = .95) #ratty = 1.199525	0.3296572	1.959116	
median_hdi(gsi_length_modpost$b_Intercept + gsi_length_modpost$b_treatment_b, .width = .95) #birdy = 0.9418224	0.05676643	1.705442	
...
##percent diference
gsi_diff_post<-median_hdi(gsi_length_modpost$b_treatment_b, .width = .95)
gsi_diff_post #birdy effect
gsi_rat_post<-median_hdi(gsi_length_modpost$b_Intercept, .width = .95) 
gsi_rat_post #ratty value

gsi_diff_post$y/((gsi_rat_post$y+gsi_rat_post$y+gsi_diff_post$y)/2)*100 #23.51% difference


#percent lower
gsi_diff_post$y/gsi_rat_post$y*100 #21.04% lower
...
gsi_length_mod %>%
  spread_draws(b_Intercept, b_treatment_bbirdy) %>%
   ...
   mutate(percent_diff = (bird_diff)/((rat+bird_diff+rat)/2), #-0.2350947
          percent_change_rat_to_bird = (bird_diff)/rat) #-0.2103667
...
#length-gsi: gsi_length_post, gsi_length_mod
#condition-length:  fult_length_post, fulton_mod_int
...
cso_dat_lfm %>% 
 mutate(treatment = factor(treatment, levels=c("ratty", "birdy")))  %>% #re-arrange to match posterior plots
  ggplot(aes(x=treatment, y=GSI, fill=treatment, shape = treatment, alpha = 0.6)) + 
  ...
  scale_y_continuous(breaks=seq(0.5, 2.5, 0.5), labels=scales::number_format(accuracy=0.1))+
  theme_bw() + 
  xlab(" ")+
  ylab("GSI") +
  theme(panel.grid.major = element_blank(), # remove gridlines
...
gsi_comb_plot <-
  ggdraw() + 
  draw_plot(gsi_boxplot, x = 0, y = 0, width = 0.5, height = 1) +
...
gw_length_mod %>%
  ...
  geom_vline(xintercept = 0, linetype = "dashed") +
  theme_bw() + 
  xlab("Difference in gonad weight (ind)")+ 
  ylab("Posterior density") +
  scale_y_continuous(breaks = NULL) +
 # scale_x_continuous(limits=c(-1.4, 0.25), breaks=seq(-1.4, 0.2, .2), labels = scales::number_format(accuracy = 0.1))+
  scale_x_continuous(limits=c(-1.5, .2), breaks=seq(-1.5, .2,.5), labels = scales::number_format(accuracy = 0.1))+
...
##plot of raw data with regression lines:
###gw-length
gw_length_plot<-
cso_dat_lfm %>%
...
#Exceprts from older scripts: Weight-Length
```{r}
...
##b estimates:
median_hdi(lw_post$b_llength, .width = .95) #b for ratty islands: 2.6518	2.417132	2.875315	
median_hdi(lw_post$`b_llength:treatment_bbirdy`, .width = .95) #interaction for b - add to b_llength to get b for birdy islands: 0.4304035	0.1699629	0.6931922	
median_hdi(lw_post$`b_llength:treatment_bbirdy` + lw_post$b_llength, .width = .95) # b for birdy islands: 3.082685	2.893422	3.278773	

###so based on this, plus Froese 2006 - for birdy islands small fish have the same form and condition as large fish (b = 3), but for ratty islands small ind were in better nutritional condition than larger individuals at time of sampling (b < 3)


#a estimates: 
median_hdi(exp(lw_post$b_Intercept), .width = .95) #intercept - needed to exp() to get a for ratty islands): 0.06007816	0.02385148	0.1105351
median_hdi(lw_post$b_treatment_bbirdy, .width = .95) #effect of birds on intercept - so add to intercept (and then exp()) to get a for birdy islands = -1.351138	-2.171899	-0.5716991
median_hdi(exp(lw_post$`b_treatment_bbirdy` + lw_post$b_Intercept), .width = .95) #a for birdy islands (exp to get on original scale): 0.01557301	0.007439915	0.0267236	
median_hdi(exp(lw_post$`b_treatment_bbirdy` + lw_post$b_Intercept) - exp(lw_post$b_Intercept), .width=0.95) # difference in a for birdy vs. ratty islands islands (exp to get on original scale): -0.04398547	-0.09338679	-0.00946873	
...
####Exceprts from older scripts: Plots of posterior distributions
```{r}
#length-weight: lw_post, lw_mod_ll_int
#length-gsi: gsi_length_post, gsi_length_mod
#condition-length:  fult_length_post, fulton_mod_int
...
##Exceprts from older scripts: Plots - now with raw data and estimated regression lines
```{r}
...
cso_dat_lfm %>%
  group_by(treatment_b) %>%
  data_grid(c_day = 0, c_length = seq_range(c_length, n = 20))%>%
  add_fitted_draws(gsi_length_day_mod, re_formula=NA) %>% #re_formula = NA to deal with random effects
  ggplot(aes(x = c_length, y = GSI, color = treatment_b, fill = treatment_b,  alpha = 0.6)) +
  geom_point(data = cso_dat_lfm, aes(x = fork_length_cm), size = 3, aes(shape = ordered(treatment_b)))+
  stat_lineribbon(aes(y = .value), .width=0) +
  scale_fill_manual(values = c( "#DC3220","#005AB5")) + 
  scale_color_manual(values = c("#DC3220", "#005AB5")) +   
  scale_shape_manual(values = c(23,21)) + 
...
cso_dat_lfm %>%
  group_by(treatment_b) %>%
  add_fitted_draws(fulton_mod_int, re_formula=NA) %>% #re_formula = NA to deal with random effects
  ggplot(aes(x = fork_length_cm, y = fulton_k, color = treatment_b, fill = treatment_b,  alpha = 0.6)) +
  geom_point(data = cso_dat_lfm, size = 3, aes(shape = ordered(treatment_b)))+
  stat_lineribbon(aes(y = .value), .width=0) +
  scale_fill_manual(values = c( "#DC3220","#005AB5")) + 
  scale_color_manual(values = c("#DC3220", "#005AB5")) +   
...
####gsi-length models: with collection date
...
hypothesis(gsi_length_day_mod, "c_length<0") #posterior probability = 0.98 that slope < 0 - slight decrease in GSI with increasing length...

###Bird/rat estimate nearly identical to model without collection day (see above)

###gsi-length models: based on moon phase rather than collection date#### 
##based on quarters:
cso_dat_lfm<-
cso_dat_lfm %>%
  mutate(moon_quart = case_when(collection_date == "05-Mar-19" ~ "FQ", 
         collection_date == "07-Mar-19"~ "FQ",
         collection_date == "09-Mar-19"~ "FQ", 
         collection_date == "10-Mar-19"~ "FQ",
         collection_date == "14-Mar-19"~ "SQ",
         collection_date == "16-Mar-19"~ "SQ"),
         moon_quart = as.factor(moon_quart))
###run model:

gsi_length_moon_mod <- 
  brm(data = cso_dat_lfm, family = gaussian,
      GSI  ~ c_length+treatment_b + moon_quart+ (1|atoll),
      prior = c(prior(normal(1,1), class = Intercept), ###weakly informative prior
                prior(normal(0, 1), class = "b", coef = "c_length"), #this DOESN'T have to be above 0, unlike gonad weight - so weakly informative
                prior(normal(0, 1), class = "b", coef = "moon_quartSQ"),
                prior(normal(0, 1), class = "b", coef = "treatment_bbirdy")), #weakly informative prior
      iter = 3000, warmup = 1000, chains = 4, cores = 4,
      control = list(adapt_delta = 0.999, max_treedepth = 15), 
      sample_prior="yes",
      file = "gsi_length_moon_mod")

print(gsi_length_moon_mod) #not much effect of moon phase. Strong effect of rat/seabird status. 
plot(gsi_length_moon_mod, ask = FALSE)
pp_check(gsi_length_moon_mod)
##all looks good

#Compile posterior distributions:
gsi_length_moon_post <- posterior_samples(gsi_length_moon_mod)
median_hdi(gsi_length_moon_post$b_treatment_bbirdy, .width = .95) #median estimate = -0.25, ymin= -0.45, ymax = -0.06
###Bird/rat estimate nearly identical to other models**


#compare to model with collection day, and with none:
loo(gsi_length_day_mod, gsi_length_moon_mod, gsi_length_mod)
model_weights(gsi_length_day_mod, gsi_length_moon_mod, gsi_length_mod,  weights = "loo") %>%
round(digits = 3)
...
model_weights(gsi_length_moon_mod, gsi_length_mod,  weights = "loo") %>%
round(digits = 3)
##nearly identical weight

model_weights(gsi_length_moon_mod, gsi_length_day_mod,  weights = "loo") %>%
round(digits = 3)
##moon model better. 
