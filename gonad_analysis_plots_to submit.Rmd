...
##Try combining all post plots:
lwa_tab<-
lw_mod_ll_int %>%
  spread_draws(b_treatment_bbirdy, b_Intercept, `b_llength:treatment_bbirdy`) %>%
  mutate(birdy_b_lw = `b_llength:treatment_bbirdy`,
          exp_birdy_lw = exp(b_Intercept + b_treatment_bbirdy) - exp(b_Intercept),
         model = "2_lwa")
...
#post_tab<-
#  cbind(lw_tab, gwl_tab, vbgf_tab)
#post_tab



  ggplot(data = lwa_tab, aes(y = model, x = birdy_b_lw, fill = stat(x < 0))) +
  stat_halfeyeh(point_interval=median_hdi, .width=.95, fatten_point = 2, slab_alpha = 0.6) +
  stat_halfeyeh(data = lwb_tab, aes(x=exp_birdy_lw), point_interval=median_hdi, .width=.95, fatten_point = 2, slab_alpha = 0.6) +
  stat_halfeyeh(data = gwl_tab, aes(x=b_treatment_bbirdy), point_interval=median_hdi, .width=.95, fatten_point = 2, slab_alpha = 0.6) +
  stat_halfeyeh(data = vbgf_tab,aes(x=b_K1_Intercept), point_interval=median_hdi, .width=.95, fatten_point = 2, slab_alpha = 0.6) +
  scale_fill_manual(values = alpha(c("#005AB5", "#DC3220"),0.3))+
  geom_vline(xintercept = 0, linetype = "dashed") +
  theme_bw() + 
  xlab("Response")+ 
...
gwl_comb_plot <-
  ggdraw() + 
  draw_plot(gw_length_plot, x = 0, y = 0, width = 0.5, height = 1) +
  draw_plot(gwl_post_plot,  x = 0.5, y = 0, width = 0.5, height = 1) 
gwl_comb_plot
...
gwl_comb_plot <-
  ggdraw() +
  draw_plot(gw_length_plot) +
  draw_plot(gwl_post_plot, x= 0.16, y = 0.65, width = .35, height = .35)

gwl_comb_plot
gwl_comb_plot <-
  plot_grid(gw_length_plot, gwl_post_plot, ncol = 2, align = "hv")
gwl_comb_plot
...
lw_comb_plot <-
  ggdraw() +
  draw_plot(lw_plot) +
  draw_plot(lwa_post_plot, x= 0.20, y = 0.65, width = .35, height = .35)+ 
  draw_plot(lwb_post_plot, x= 0.65, y = 0.11, width = .35, height = .35)
...
gw_ww_plots_comb<-plot_grid(gw_length_plot, lw_plot, gwl_comb_plot, gwa_comb_plot, ncol = 1, align = "v")
gw_ww_plots_comb



ggsave(filename = "gw_ww_combined_plots.pdf", 
       plot = gw_ww_plots_comb,
       width = 15, 
       height = 45,
       units = "cm",
       dpi = 300)

####may need to adjust scales/axis ticks, especially for gonad weight posterior plots 
